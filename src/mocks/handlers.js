...
const getBatteries = ({channelIds, batchIds, startDate, endDate}) => batteries.filter(battery => {
...
export const handlers = [
  rest.get('/api/batches', (req, res, ctx) => {
    ...
    return res(
      ctx.json(results.map(b => {
        const _batteries = batteries.filter(bat => bat.batch === `${b.channel}-${b.date}`)
        return {
          ...b,
  ...
  rest.get('/api/channel_report', (req, res, ctx) => {
    ...
    let channelIds = req.url.searchParams.getAll('channelId')
    
    const [startDate, endDate] = getStartAndEndDate(req.url.searchParams)
    if(!startDate || !endDate){
      return res(ctx.status(400), ctx.json({error: 'startDate and endDate are required'}))
    }
    ...
    const report = {
      ...
      byChannel: channels.map(channel => ({
        channel: channel.id,
        passing: passing.filter(battery => battery.batch.startsWith(channel.id)).length,
        failing: failing.filter(battery => battery.batch.startsWith(channel.id)).length,
      }))
    }
